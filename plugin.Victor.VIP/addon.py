...
def addLinkItem(name, url, mode, epgname=None, iconimage=None, infoLabels=False, IsPlayable=True,fanart=None):
    ...
    if epgname:
        commands = []
        commands.append(( 'Program TV', 'RunPlugin(plugin://plugin.Victor.VIP?mode=TELEWIZJADA_EPG&ex_link=%s)'%epgname ))
...
def m3u2list(url):
    url = 'https://www.easypaste.org/file/YoRmIk/tv.kopia.m3u?lang=pl'
